...
export default async function MaterialEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [materialResult, suppliersResult, seasonsResult, colorsResult] = await Promise.all([
    supabase
      .from("materials")
    ...
    supabase.from("materials").select("color").not("color", "is", null),
  ]);
  ...
  const pastColors = Array.from(
    new Set((colorsResult.data ?? []).map((r: any) => r.color).filter(Boolean))
  ) as string[];
