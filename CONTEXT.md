...
- **ADR-0011 Phase 2 = 完了（本番反映済み）**。旧 `/models` scaffolding（gender 直持ち・version なし）を作り直し:
  ...
  - **status は Active / Deprecated の2種のみ**（DB は active/frozen/deprecated のままだが、frozen は UI 上 Active 扱い。partial unique「1 season 1 active」は DB status で維持）。**Lock/Unlock は別軸で導出**: `Deprecated → 常に Locked`／`Active → その版の product に ProductionBatch が生成済みなら Locked`（`production_batches.product_id → products.model_version_id`）、未なら Unlocked。**編集可否＝この lock**（`VersionRow.locked`。Locked/deprecated のみ read-only）。UI: 行頭に **lock アイコン**、Season 下に status。「Frozen」表記は廃止。Batch→自動 Lock は導出なので即時反映（現状 3 batch 版＋deprecated が Locked）。
  - **Model Versions ページ**（`/model-versions`, サイドバー Products>Versions）: 全版を model でグループ表示（`VersionsTable` 共有）。checkbox＋Bulk（**Archive=Deprecate / Unarchive=復帰 / Delete**（product 持ちは skip））。一覧/詳細の版取得は `loadAllVersionRows`（ページング）/`loadVersionRows`。
...
- **Phase 3（Product ページの Model 連携）**。**決定済**: recipe は **dual-write 維持＋表示カード**（読み替えは Phase 4）、作成は **フル Model/Version ピッカー**。実装単位:
  - **3a データ層 = 完了**（コミット `84b1334`）: `resolveModelVersion(name, category, seasonId)` を追加。**Model 照合は正規化名（大小文字/空白無視）× category**でマージ survivor に一致させ、無ければ新規作成。Version は「その season の版→最新版→無ければ当該 season の空 active 版を新規作成」。`createProduct` が resolve して `model_id`＋`model_version_id` をセット、`duplicateProduct` は source からコピー（`DUPLICATE_FIELDS` に追加）。`updateProduct` は既存リンク保護のため未変更（明示的な貼り替えは 3b のピッカーで）。`model_name` は非正規化コピーとして維持（検索・表示・material-usage 用）。
    - **既知の残課題（正規化しても照合できない実データ ≈40件 / 1921）**: `products.model_name` が紐づく Model の名前と **綴り・語形・略記の違い**でズレているもの（**category 差ではない。category_drift=0**）。例: `mountain parka`↔`m.parka`、`highneck`↔`high neck`、`shirts`↔`shirt`、`work blazer SL`↔`work blazer semi-long`、`DOUBLE BRESTED`↔`DOUBLE BREASTED`（タイポ）。case/space の正規化では吸収できないため、**この綴りのまま新規 createProduct すると重複 Model が作られ得る**（Duplicate は model_id をコピーするので安全／既存リンクも保護）。3b のピッカーで選択を明示化すれば回避。これら ≈40 件は手動レビュー or Merge ツールで整理する候補（別途対応）。
  ...
  - **Phase 4（詳細設計＝完了 / 実装＝進行中。決定は ADR-0011 §9）**: 裏地(色含む)/非メイン素材/サイズ/組成を **Version 所有**へ（Product は**量産バッチ生成まで自動同期する snapshot、生成後は凍結**＝`EXISTS(production_batches)` から導出。新カラム/`status='final'` 流用なし）。**製造時間は Product 所有のまま維持**（実データで per-product 調整＝427グループ中62で製造時間が異なるのを確認。Model はテンプレ）。recipe 編集で**量産前 Product に cost をライブ再計算**（現在 set_cost・原価式＝メイン(Product)+非メイン(Version)+製造(Product)・retail 不変）、fan-out は**単一 RPC トランザクション＋確認画面**（商品別 現在値→再計算値→差額、±20%超を強調、キャンセル可）。Product Edit は Lining/組成/非メイン素材/サイズの編集を撤去し**読取「Model Recipe」カード**化（Phase 3c を統合）、Cost UI は編集可＝メイン数量+製造/読取＝Version 由来の非メイン。dry-run で **recipe 分岐 0/1921**（切替は cost no-op）。作成/Duplicate は Version からレシピ snapshot（Duplicate 後にメイン素材が変わったら製造時間の確認表示、新規 Model の空 Version は「レシピ未定義」明示）。
    - **Phase 4-1（copy-on-create）＝実装済み**: `syncProductRecipeFromVersion`（作成/Duplicate 時に Version のレシピ＋製造テンプレを Product へ snapshot、量産前のみ）。
    - **Phase 4-2（recipe 編集の cost 伝播）＝実装済み（2026-08-17・書き込み側まで）**: 単一 Postgres 関数 **`apply_model_version_recipe(p_version_id, p_changelog, p_orderable_sizes, p_accessory_composition, p_minutes, p_materials, p_dry_run)`**（`security definer`＋`is_internal()`）で version 編集＋量産前 Product 全件の再同期・**カラー単位 cost ライブ再計算**を**1トランザクション all-or-nothing**（`p_dry_run=true`＝preview／`false`＝apply、同一 SQL 経路で preview と結果がズレない）。製造時間は Product 所有のため触らず、mfg 成分は `cost_jpy−material_cost_jpy` として温存、retail 不変。UI＝`ModelVersionEditModal` の Save で dry-run→**確認モーダル**（品番・現在→新・差額・合計・±20%強調、Cancel は DB 未書き込みでフォーム編集状態を保持）→ Apply で原子的適用。**関数は本番作成済み（migration `20260816150630`）**だが **UI からの実 Apply は未実行**（Apply すると §9.8 の 212 件 set-cost ドリフトがフラッシュされ元に戻せないため、ドリフト棚卸し時にまとめて実施）。rollback-test（自己中断Tx で書込→検証→ROLLBACK、`cost_written_matches_preview=true`）＋ tsc/build/vitest 緑で検証済み。
    - **Phase 4 §9.7 / §3.7 ＝実装済み（2026-08-17・スライス1〜4）**: Product Edit を **読取「Model Recipe」カード**化（`getProductRecipeCard` + `ProductModelRecipeCard`＝裏地/非メイン+用尺/サイズ/組成/製造テンプレ+非メイン小計を Version 由来で読取、「Model 版を編集 →」で `ModelVersionEditModal`）。Basic Info から Lining/Orderable Sizes/Accessories の**編集を撤去**（`updateProduct` は `VERSION_OWNED_PRODUCT_FIELDS` を patch から除外＝version 同期スナップショットの null 上書き防止）。Cost フォームは編集可＝メイン数量+製造時間／**非メインは snapshot から読取**（`updateProductCosts` は `liningQuantity`/`additionalRows` 引数を廃し、`product_materials`/`lining_m_quantity` を書かず非メインを snapshot から算出）。**staleness（§3.7）**: **old version**＝**季節順**で「より後の季節の版が存在」時にカード右肩バッジ（`src/lib/season-order.ts` `seasonSortKey`＝YY.1/.2・YYSS/AW・YYPOP を1本の時系列に、`委託`/`ALLSS` は agnostic=null。**created_at ではなく季節順**——backfill は season 順と無関係に created_at が付くため）。**old price**＝stored 材料原価 vs 現在 set_cost 再計算の差を Cost カードにバッジ＋バナー（**可視化のみ・自動更新なし**、保存時に再計算でフラッシュ）。書き込み経路は保存ガードのみ変更、tsc/build/vitest 38 緑。
    - **Phase 4 残り＝未着手**: set-cost staleness の**自動化**（§9.8 の 212 件棚卸し——±20%超75件・stored=0の2件の精査・修正。**調査結果＝`docs/adr/0011-drift-stocktake.md`**: A=価格NULL 11件／B=ZZZ titan frame 単位ミス疑い 54件／C=DNA paper denim 重複疑い 2件／D=再計算安全 8件／stored=0 安全 2件）と、UI からの Version 一括 Apply（ドリフトフラッシュを伴うため棚卸しと同時に実施）。
- **以降**: Phase 4 残り（set-cost staleness 自動化・212件棚卸し）→ Phase 5（Retail ガイド更新）→ Phase 6（Deprecation UX の影響 Product 警告一覧）。
...
- **データ品質メモ（Phase 4 cost 伝播の設計中に dry-run で判明・今回スコープ外・2026-08-15）**: version-linked product **1,921 件**で「stored 原価 vs 現在 set_cost で再計算」を比較。**recipe 分岐は 0 件**（＝非メイン素材/裏地/サイズ/組成を Version 所有へ切り替えても cost に対して no-op で安全。バックフィルの signature 化・Phase 3a の name 照合・69件の重複レシピ統合を経ても Product と Version のレシピは全件一致）。ただし **212 件で stored 原価が現在 set_cost の再計算値とズレ**（ADR §3.7「old price」staleness。Model/Version 化とは無関係の既存状態・最大 ≈¥38,094）: **210 件が set cost ドリフト**（うち **75 件は ±20% 超**——例 `DOUBLE BREASTED WORK COAT` ¥42,384→¥4,290 の約9割減。**単位ミス／素材差し替え等の素材データ異常の可能性**があり要精査）、**2 件は stored=0 のまま原価未計算のデータ穴**（例 品番2304 `-moment- m.parka origami sleeve`）。set-cost staleness 自動化（Phase 4）着手時にまとめて棚卸しする。
