...
- **Phase 3（Product ページの Model 連携）**。**決定済**: recipe は **dual-write 維持＋表示カード**（読み替えは Phase 4）、作成は **フル Model/Version ピッカー**。実装単位:
  ...
  - **Phase 4（詳細設計＝完了 / 実装＝進行中。決定は ADR-0011 §9）**: 裏地(色含む)/非メイン素材/サイズ/組成を **Version 所有**へ（Product は**量産バッチ生成まで自動同期する snapshot、生成後は凍結**＝`EXISTS(production_batches)` から導出。新カラム/`status='final'` 流用なし）。**製造時間は Product 所有のまま維持**（実データで per-product 調整＝427グループ中62で製造時間が異なるのを確認。Model はテンプレ）。recipe 編集で**量産前 Product に cost をライブ再計算**（現在 set_cost・原価式＝メイン(Product)+非メイン(Version)+製造(Product)・retail 不変）、fan-out は**単一 RPC トランザクション＋確認画面**（商品別 現在値→再計算値→差額、±20%超を強調、キャンセル可）。Product Edit は Lining/組成/非メイン素材/サイズの編集を撤去し**読取「Model Recipe」カード**化（Phase 3c を統合）、Cost UI は編集可＝メイン数量+製造/読取＝Version 由来の非メイン。dry-run で **recipe 分岐 0/1921**（切替は cost no-op）。作成/Duplicate は Version からレシピ snapshot（Duplicate 後にメイン素材が変わったら製造時間の確認表示、新規 Model の空 Version は「レシピ未定義」明示）。
    ...
    - **Phase 4 残り＝未着手**: version 読み替え（Product Edit の Lining/組成/非メイン/サイズを撤去し **§9.7 読取「Model Recipe」カード**へ）と staleness flags（§3.7 old version / old price の可視化・set-cost staleness 自動化）。
- **以降**: Phase 4 残り（Product Edit 読取カード化・staleness）→ Phase 5（Retail ガイド更新）→ Phase 6（Deprecation UX の影響 Product 警告一覧）。
